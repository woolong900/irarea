...
const product = getProduct('coin612')
const productImage = product ? getProductMainImage(product.id) : ''

export const metadata: Metadata = {
  title: product ? `${product.name} - ${product.subtitle}` : 'COIN612 Thermal Module',
  ...
  alternates: {
    canonical: `https://irarea.com/coin612`,
  },
}
...
export default async function Coin612Page({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)
  
  const localizedProduct = getLocalizedProduct('coin612', locale as Locale)
  const relatedProducts = getLocalizedRelatedProducts('coin612', locale as Locale, 4)
