...
const product = getProduct('plug1212r')
const productImage = product ? getProductMainImage(product.id) : ''

export const metadata: Metadata = {
  title: product ? `${product.name} - ${product.subtitle}` : 'PLUG1212R Thermal Module',
  ...
  alternates: {
    canonical: `https://irarea.com/plug1212r`,
  },
}
...
export default async function Plug1212RPage({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)
  
  const localizedProduct = getLocalizedProduct('plug1212r', locale as Locale)
  const relatedProducts = getLocalizedRelatedProducts('plug1212r', locale as Locale, 4)
