...
const product = getProduct('plug612r')
const productImage = product ? getProductMainImage(product.id) : ''

export const metadata: Metadata = {
  title: product ? `${product.name} - ${product.subtitle}` : 'PLUG612R Thermal Module',
  ...
  alternates: {
    canonical: `https://irarea.com/plug612r`,
  },
}
...
export default async function Plug612RPage({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)
  
  const localizedProduct = getLocalizedProduct('plug612r', locale as Locale)
  const relatedProducts = getLocalizedRelatedProducts('plug612r', locale as Locale, 4)
