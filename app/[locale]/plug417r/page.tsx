...
const product = getProduct('plug417r')
const productImage = product ? getProductMainImage(product.id) : ''

export const metadata: Metadata = {
  title: product ? `${product.name} - ${product.subtitle}` : 'PLUG417R Thermal Module',
  ...
  alternates: {
    canonical: `https://irarea.com/plug417r`,
  },
}
...
export default async function Plug417RPage({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)
  
  const localizedProduct = getLocalizedProduct('plug417r', locale as Locale)
  const relatedProducts = getLocalizedRelatedProducts('plug417r', locale as Locale, 4)
