...
const product = getProduct('xh1212ma')
const productImage = product ? getProductMainImage(product.id) : ''

export const metadata: Metadata = {
  title: product ? `${product.name} - ${product.subtitle}` : 'XH1212MA Thermal Module',
  ...
  alternates: {
    canonical: `https://irarea.com/xh1212ma`,
  },
}

export default async function Xh1212MaPage({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)

  const localizedProduct = getLocalizedProduct('xh1212ma', locale as Locale)
  const relatedProducts = getLocalizedRelatedProducts('xh1212ma', locale as Locale)
