...
const product = getProduct('iha417w')
const productImage = product ? getProductMainImage(product.id) : ''

export const metadata: Metadata = {
  title: product ? `${product.name} - ${product.subtitle}` : 'IHA417W Thermal Module',
  ...
  alternates: {
    canonical: `https://irarea.com/iha417w`,
  },
}
...
export default async function Iha417WPage({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)
  
  const localizedProduct = getLocalizedProduct('iha417w', locale as Locale)
  const relatedProducts = getLocalizedRelatedProducts('iha417w', locale as Locale, 4)
