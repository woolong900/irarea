...
const product = getProduct('coin417g2')
const productImage = product ? getProductMainImage(product.id) : ''

export const metadata: Metadata = {
  title: product ? `${product.name} - ${product.subtitle}` : 'COIN417G2 Thermal Module',
  ...
  alternates: {
    canonical: `https://irarea.com/coin417g2`,
  },
}
...
export default async function Coin417G2Page({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)
  
  const localizedProduct = getLocalizedProduct('coin417g2', locale as Locale)
  const relatedProducts = getLocalizedRelatedProducts('coin417g2', locale as Locale, 4)
