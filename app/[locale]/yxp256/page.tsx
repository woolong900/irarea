...
const product = getProduct('yxp256')
const productImage = product ? getProductMainImage(product.id) : ''

export const metadata: Metadata = {
  title: product ? `${product.name} - ${product.subtitle}` : 'YXP256 Thermal Module',
  ...
  alternates: {
    canonical: `https://irarea.com/yxp256`,
  },
}
...
export default async function Yxp256Page({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)

  const localizedProduct = getLocalizedProduct('yxp256', locale as Locale)
  const relatedProducts = getLocalizedRelatedProducts('yxp256', locale as Locale)
