...
const product = getProduct('mini212')
const productImage = product ? getProductMainImage(product.id) : ''

export const metadata: Metadata = {
  title: product ? `${product.name} - ${product.subtitle}` : 'MINI212 Thermal Module',
  description: product?.description?.substring(0, 155) + '...' || 'High-performance thermal imaging product from IR AREA.',
  keywords: product ? [product.name, product.category, 'thermal imaging', 'infrared', 'IR AREA', 'miniature drone thermal', 'UAV thermal camera', 'ultra-lightweight IR module', 'drone payload camera'] : [],
  openGraph: {
  ...
  alternates: {
    canonical: `https://irarea.com/mini212`,
  },
}
...
export default async function Mini212Page({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)
  
  const localizedProduct = getLocalizedProduct('mini212', locale as Locale)
  const relatedProducts = getLocalizedRelatedProducts('mini212', locale as Locale, 4)
