...
const product = getProduct('yxi256')
const productImage = product ? getProductMainImage(product.id) : ''

export const metadata: Metadata = {
  title: product ? `${product.name} - ${product.subtitle}` : 'YXI256 Thermal Module',
  ...
  alternates: {
    canonical: `https://irarea.com/yxi256`,
  },
}
...
export default async function Yxi256Page({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)

  const localizedProduct = getLocalizedProduct('yxi256', locale as Locale)
  const relatedProducts = getLocalizedRelatedProducts('yxi256', locale as Locale)
