...
const product = getProduct('yxw256')
const productImage = product ? getProductMainImage(product.id) : ''

export const metadata: Metadata = {
  title: product ? `${product.name} - ${product.subtitle}` : 'YXW256 Thermal Module',
  ...
  alternates: {
    canonical: `https://irarea.com/yxw256`,
  },
}
...
export default async function Yxw256Page({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)

  const localizedProduct = getLocalizedProduct('yxw256', locale as Locale)
  const relatedProducts = getLocalizedRelatedProducts('yxw256', locale as Locale)
