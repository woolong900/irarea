...
const product = getProduct('twin1212')
const productImage = product ? getProductMainImage(product.id) : ''

export const metadata: Metadata = {
  title: product ? `${product.name} - ${product.subtitle}` : 'TWIN1212 Thermal Module',
  ...
  alternates: {
    canonical: `https://irarea.com/twin1212`,
  },
}
...
export default async function Twin1212Page({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)
  
  const localizedProduct = getLocalizedProduct('twin1212', locale as Locale)
  const relatedProducts = getLocalizedRelatedProducts('twin1212', locale as Locale, 4)
