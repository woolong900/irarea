...
const product = getProduct('xh1210lc')
const productImage = product ? getProductMainImage(product.id) : ''

export const metadata: Metadata = {
  title: product ? `${product.name} - ${product.subtitle}` : 'XH1210LC Thermal Module',
  ...
  alternates: {
    canonical: `https://irarea.com/xh1210lc`,
  },
}

export default async function Xh1210LcPage({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)

  const localizedProduct = getLocalizedProduct('xh1210lc', locale as Locale)
  const relatedProducts = getLocalizedRelatedProducts('xh1210lc', locale as Locale)
