...
const product = getProduct('min1212r')
const productImage = product ? getProductMainImage(product.id) : ''

export const metadata: Metadata = {
  title: product ? `${product.name} - ${product.subtitle}` : 'MIN1212R Thermal Module',
  ...
  alternates: {
    canonical: `https://irarea.com/min1212r`,
  },
}
...
export default async function Min1212RPage({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)
  
  const localizedProduct = getLocalizedProduct('min1212r', locale as Locale)
  const relatedProducts = getLocalizedRelatedProducts('min1212r', locale as Locale, 4)
