...
const product = getProduct('igs412')
const productImage = product ? getProductMainImage(product.id) : ''

export const metadata: Metadata = {
  title: product ? `${product.name} - ${product.subtitle}` : 'IGS412 Thermal Module',
  description: product?.description?.substring(0, 155) + '...' || 'High-performance thermal imaging product from IR AREA.',
  keywords: product ? [product.name, product.category, 'thermal imaging', 'infrared', 'IR AREA', 'drone thermal camera', 'UAV infrared module', 'lightweight thermal core', 'drone gimbal camera'] : [],
  openGraph: {
  ...
  alternates: {
    canonical: `https://irarea.com/igs412`,
  },
}
...
export default async function Igs412Page({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)
  
  const localizedProduct = getLocalizedProduct('igs412', locale as Locale)
  const relatedProducts = getLocalizedRelatedProducts('igs412', locale as Locale, 4)
