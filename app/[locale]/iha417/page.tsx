...
const product = getProduct('iha417')
const productImage = product ? getProductMainImage(product.id) : ''

export const metadata: Metadata = {
  title: product ? `${product.name} - ${product.subtitle}` : 'IHA417 Thermal Module',
  ...
  alternates: {
    canonical: `https://irarea.com/iha417`,
  },
}
...
export default async function Iha417Page({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)
  
  const localizedProduct = getLocalizedProduct('iha417', locale as Locale)
  const relatedProducts = getLocalizedRelatedProducts('iha417', locale as Locale, 4)
