...
const product = getProduct('plug617r')
const productImage = product ? getProductMainImage(product.id) : ''

export const metadata: Metadata = {
  title: product ? `${product.name} - ${product.subtitle}` : 'PLUG617R Thermal Module',
  ...
  alternates: {
    canonical: `https://irarea.com/plug617r`,
  },
}
...
export default async function Plug617RPage({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)
  
  const localizedProduct = getLocalizedProduct('plug617r', locale as Locale)
  const relatedProducts = getLocalizedRelatedProducts('plug617r', locale as Locale, 4)
