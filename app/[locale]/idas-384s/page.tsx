...
const product = getProduct('idas-384s')
const productImage = product ? getProductMainImage(product.id) : ''

export const metadata: Metadata = {
  title: product ? `${product.name} - ${product.subtitle}` : 'IDAS-384S Thermal Module',
  ...
  alternates: {
    canonical: `https://irarea.com/idas-384s`,
  },
}
...
export default async function Idas384SPage({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)
  
  const localizedProduct = getLocalizedProduct('idas-384s', locale as Locale)
  const relatedProducts = getLocalizedRelatedProducts('idas-384s', locale as Locale, 4)
