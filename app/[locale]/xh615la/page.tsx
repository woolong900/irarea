...
const product = getProduct('xh615la')
const productImage = product ? getProductMainImage(product.id) : ''

export const metadata: Metadata = {
  title: product ? `${product.name} - ${product.subtitle}` : 'XH615LA Thermal Module',
  ...
  alternates: {
    canonical: `https://irarea.com/xh615la`,
  },
}

export default async function Xh615LaPage({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)

  const localizedProduct = getLocalizedProduct('xh615la', locale as Locale)
  const relatedProducts = getLocalizedRelatedProducts('xh615la', locale as Locale)
