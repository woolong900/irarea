...
const product = getProduct('twin612')
const productImage = product ? getProductMainImage(product.id) : ''

export const metadata: Metadata = {
  title: product ? `${product.name} - ${product.subtitle}` : 'TWIN612 Thermal Module',
  ...
  alternates: {
    canonical: `https://irarea.com/twin612`,
  },
}
...
export default async function Twin612Page({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)
  
  const localizedProduct = getLocalizedProduct('twin612', locale as Locale)
  const relatedProducts = getLocalizedRelatedProducts('twin612', locale as Locale, 4)
