...
const product = getProduct('yxi96')
const productImage = product ? getProductMainImage(product.id) : ''

export const metadata: Metadata = {
  title: product ? `${product.name} - ${product.subtitle}` : 'YXI96 Thermal Module',
  ...
  alternates: {
    canonical: `https://irarea.com/yxi96`,
  },
}
...
export default async function Yxi96Page({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)

  const localizedProduct = getLocalizedProduct('yxi96', locale as Locale)
  const relatedProducts = getLocalizedRelatedProducts('yxi96', locale as Locale)
