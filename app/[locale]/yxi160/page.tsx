...
const product = getProduct('yxi160')
const productImage = product ? getProductMainImage(product.id) : ''

export const metadata: Metadata = {
  title: product ? `${product.name} - ${product.subtitle}` : 'YXI160 Thermal Module',
  ...
  alternates: {
    canonical: `https://irarea.com/yxi160`,
  },
}
...
export default async function Yxi160Page({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)
  
  const localizedProduct = getLocalizedProduct('yxi160', locale as Locale)
  const relatedProducts = getLocalizedRelatedProducts('yxi160', locale as Locale, 4)
