...
const product = getProduct('xh615ma')
const productImage = product ? getProductMainImage(product.id) : ''

export const metadata: Metadata = {
  title: product ? `${product.name} - ${product.subtitle}` : 'XH615MA Thermal Module',
  ...
  alternates: {
    canonical: `https://irarea.com/xh615ma`,
  },
}

export default async function Xh615MaPage({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)

  const localizedProduct = getLocalizedProduct('xh615ma', locale as Locale)
  const relatedProducts = getLocalizedRelatedProducts('xh615ma', locale as Locale)
