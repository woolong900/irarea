...
const product = getProduct('xh1210hmf')
const productImage = product ? getProductMainImage(product.id) : ''

export const metadata: Metadata = {
  title: product ? `${product.name} - ${product.subtitle}` : 'XH1210HMF Thermal Module',
  ...
  alternates: {
    canonical: `https://irarea.com/xh1210hmf`,
  },
}

export default async function Xh1210HmfPage({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)

  const localizedProduct = getLocalizedProduct('xh1210hmf', locale as Locale)
  const relatedProducts = getLocalizedRelatedProducts('xh1210hmf', locale as Locale)
