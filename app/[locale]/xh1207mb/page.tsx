...
const product = getProduct('xh1207mb')
const productImage = product ? getProductMainImage(product.id) : ''

export const metadata: Metadata = {
  title: product ? `${product.name} - ${product.subtitle}` : 'XH1207MB Thermal Module',
  ...
  alternates: {
    canonical: `https://irarea.com/xh1207mb`,
  },
}

export default async function Xh1207MbPage({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)

  const localizedProduct = getLocalizedProduct('xh1207mb', locale as Locale)
  const relatedProducts = getLocalizedRelatedProducts('xh1207mb', locale as Locale)
