...
const product = getProduct('yxp160')
const productImage = product ? getProductMainImage(product.id) : ''

export const metadata: Metadata = {
  title: product ? `${product.name} - ${product.subtitle}` : 'YXP160 Thermal Module',
  ...
  alternates: {
    canonical: `https://irarea.com/yxp160`,
  },
}
...
export default async function Yxp160Page({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)

  const localizedProduct = getLocalizedProduct('yxp160', locale as Locale)
  const relatedProducts = getLocalizedRelatedProducts('yxp160', locale as Locale)
