...
const product = getProduct('xh615hmg')
const productImage = product ? getProductMainImage(product.id) : ''

export const metadata: Metadata = {
  title: product ? `${product.name} - ${product.subtitle}` : 'XH615HMG Thermal Module',
  ...
  alternates: {
    canonical: `https://irarea.com/xh615hmg`,
  },
}

export default async function Xh615HmgPage({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)

  const localizedProduct = getLocalizedProduct('xh615hmg', locale as Locale)
  const relatedProducts = getLocalizedRelatedProducts('xh615hmg', locale as Locale)
