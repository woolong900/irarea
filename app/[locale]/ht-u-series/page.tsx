...
const product = getProduct('ht-u-series')
const productImage = product ? getProductMainImage(product.id) : ''

export const metadata: Metadata = {
  title: product ? `${product.name} - ${product.subtitle}` : 'HT U01/U02/U03 Drone Infrared Camera Module',
  description: product?.description?.substring(0, 155) + '...' || 'High-performance drone infrared camera module from IR AREA.',
  keywords: product ? [product.name, product.category, 'thermal imaging', 'infrared', 'IR AREA', 'drone infrared camera', 'UAV thermal module'] : [],
  openGraph: {
  ...
  alternates: {
    canonical: `https://irarea.com/ht-u-series`,
  },
}

export default async function HtUSeriesPage({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)

  const localizedProduct = getLocalizedProduct('ht-u-series', locale as Locale)
  const relatedProducts = getLocalizedRelatedProducts('ht-u-series', locale as Locale)
