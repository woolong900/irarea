...
const product = getProduct('ilc212')
const productImage = product ? getProductMainImage(product.id) : ''

export const metadata: Metadata = {
  title: product ? `${product.name} - ${product.subtitle}` : 'ILC212 Thermal Module',
  ...
  alternates: {
    canonical: `https://irarea.com/ilc212`,
  },
}
...
export default async function Ilc212Page({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)
  
  const localizedProduct = getLocalizedProduct('ilc212', locale as Locale)
  const relatedProducts = getLocalizedRelatedProducts('ilc212', locale as Locale, 4)
